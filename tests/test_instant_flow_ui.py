...
class TestInstantFlowUi(unittest.TestCase):
    ...
    def test_percent_cut_off_paints_before_release(self) -> None:
        ...
        self.assertIn('resume_percent_cut_live', resume)
        self.assertNotIn('_ensure_network_context_for_victim', resume)
        self.assertNotIn('reinforce_restore', resume)
        self.assertNotIn('_release_victim_arp_mitm_stack', resume)
        stop = method_src('stopPercentCut')
