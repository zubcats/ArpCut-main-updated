...
def _control_panel_log_targets(basename: str):
...
def _write_control_panel_boot_chunk(extra_lines):
    stamp = datetime.now().isoformat(timespec='seconds')
    lines = [
        stamp + ' rthook_qt_control_panel (PyInstaller, pre-PyQt)',
    ...
    body = '\n'.join(lines) + '\n\n'
    for path in _control_panel_log_targets('zubcut_control_panel_boot.txt'):
        try:
...
_write_control_panel_boot_chunk(['(before Qt plugin search)'])
...
if getattr(sys, 'frozen', False):
    ...
    for _root in _roots:
        ...
        if _found:
            break
    _write_control_panel_boot_chunk(
        [
