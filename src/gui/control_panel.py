...
from constants import CONTROL_PANEL_DISPLAY_NAME, CONTROL_PANEL_UPDATE_URL
...
def _control_panel_qss() -> str:
    ...
    mute = '#9a9a9a'
    return f"""
QMainWindow#zubcutControlPanel {{
    background-color: {panel};
}}
QMainWindow#zubcutControlPanel QWidget {{
    color: {text};
    background-color: transparent;
}}
QMainWindow#zubcutControlPanel QLabel {{
    color: {text};
}}
QMainWindow#zubcutControlPanel QPushButton {{
...
QMainWindow#zubcutControlPanel QPushButton:hover {{
    background-color: {hover};
    border: 1px solid {hover};
}}
QMainWindow#zubcutControlPanel QPushButton:pressed {{
    background-color: {press};
    border: 1px solid {press};
}}
QMainWindow#zubcutControlPanel QTableWidget {{
...
QMainWindow#zubcutControlPanel QHeaderView::section {{
    background-color: #000000;
    color: {mute};
    border: none;
...
QMainWindow#zubcutControlPanel QGroupBox#cloudSyncGroup {{
    border: 1px solid {border};
    margin-top: 10px;
    padding-top: 6px;
    border-radius: 4px;
}}
QMainWindow#zubcutControlPanel QGroupBox#cloudSyncGroup::title {{
...
class ControlPanelWindow(FramelessResizableMixin, QMainWindow):
    def __init__(self, icon):
        ...
        self.setWindowTitle(CONTROL_PANEL_DISPLAY_NAME)
    ...
    def _build_ui(self):
        ...
        self.crash_panel = CrashReportsPanel(self)
        self.tabs.addTab(self.crash_panel, 'Crash reports')
    ...
    def _build_accounts_tab(self) -> QWidget:
        ...
        cloud = QGroupBox('Cloud sign-in sync (optional)', page)
        cloud.setObjectName('cloudSyncGroup')
        cloud_lay = QVBoxLayout(cloud)
        cloud_lay.setSpacing(6)
        cloud_lay.addWidget(
            QLabel(
                'Link ZubCut Control Panel to your Cloudflare Worker: new accounts and changes '
        ...
        btn_row = QHBoxLayout()
        self.btnRefresh = QPushButton('Refresh', page)
        self.btnCreate = QPushButton('Create Account', page)
        self.btnRenew = QPushButton('Renew Selected', page)
        self.btnRevoke = QPushButton('Revoke Selected', page)
        self.btnActivate = QPushButton('Activate Selected', page)
        self.btnDelete = QPushButton('Delete account', page)
        self.btnExportKv = QPushButton('Export KV file (manual)', page)
        self.btnUpdatePanel = QPushButton('Install latest build', page)
        self.btnViewCrashes = QPushButton('View crash reports', page)
        for b in (
            self.btnRefresh,
            self.btnCreate,
            self.btnRenew,
        ...
        self.btnUpdatePanel.clicked.connect(self.install_latest_build)
        self.btnViewCrashes.clicked.connect(self._view_crash_reports_for_account)
    ...
    def delete_selected_account(self) -> None:
        ...
        confirm = QMessageBox.question(
            self,
            'Delete account',
            f'Delete this account permanently from ZubCut Control Panel?\n\n'
            f'User: {user}\nLicense ID: {lic_id}\n\n'
    ...
    def _view_crash_reports_for_account(self) -> None:
        user = self._selected_user_display().strip().lower()
        if not user:
            QMessageBox.information(self, CONTROL_PANEL_DISPLAY_NAME, 'Select an account first.')
            return
        self.crash_panel.refresh()
        self.crash_panel.set_account_filter(user)
    ...
    def install_latest_build(self):
        ...
        confirm = QMessageBox.question(
            ...
            'Download and run the latest ZubCut Control Panel installer?\nContinue?',
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.Yes,
        )
        if confirm != QMessageBox.Yes:
        ...
        try:
            ...
            QMessageBox.critical(self, 'Update failed', f'Could not download or launch installer.\n{e}')
        finally:
