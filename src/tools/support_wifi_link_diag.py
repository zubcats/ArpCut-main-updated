...
def format_wifi_link_report(
    ...
    *,
    ...
    lines.append(
        '  If Kill does nothing on Wi-Fi: disable AP/client isolation on the router, '
        'prefer Ethernet PC + console, or use Mobile Hotspot (Clumsy mode).'
    )
    lines.append(
        '  WPA3 / mesh / band-steering often break ARP MITM even when this PC looks fine.'
    )
    lines.append(
        '  Wi-Fi 7 MLO (multi-link) + WPA3/PMF can also break ARP MITM — prefer WPA2, '
        'disable MLO on the router, or use Ethernet PC + console / Mobile Hotspot.'
    )
    lines.append('========================================================================')
    if raw.strip():
        lines.append('')
