...
def _run_support_diag_and_exit() -> None:
    """Support: full network/Npcap/Clumsy diagnostic without launching the GUI."""
    import tempfile
    from pathlib import Path

    # Windowed frozen builds have no console — attach one so the summary is visible.
    if _sys.platform.startswith('win') and getattr(_sys, 'frozen', False):
        try:
            import ctypes
            ...
            try:
                ...
                _sys.stderr = open('CONOUT$', 'w', encoding='utf-8', errors='replace')
            except OSError:
                pass
        except Exception:
            pass
    ...
    tools_dirs.append(here.parent / 'tools')
    if getattr(_sys, 'frozen', False):
        exe_dir = Path(_sys.executable).resolve().parent
        tools_dirs.insert(0, exe_dir)
        meipass = getattr(_sys, '_MEIPASS', None)
        if meipass:
            tools_dirs.insert(0, Path(meipass))
    for d in tools_dirs:
        ds = str(d)
        if d.is_dir() and ds not in _sys.path:
            _sys.path.insert(0, ds)

    try:
        import zubcut_support_diag as diag  # type: ignore
    except Exception as exc:
        msg = (
            'ZubCut support diagnostic unavailable.\n'
            f'{exc}\n\n'
            'Fallback: run tools\\Run-ZubCut-Support-Diag.bat from the repo, '
            'or update ZubCut and retry: ZubCut.exe --support-diag'
        )
        print(msg)
        try:
            out = Path(tempfile.gettempdir()) / 'ZubCut-Support-Diag-ERROR.txt'
            out.write_text(msg + '\n', encoding='utf-8')
            print(f'Wrote {out}')
    ...
    diag_argv = [a for a in argv[1:] if a != '--support-diag']
    code = int(diag.main(diag_argv) or 0)

    # Open the newest Desktop report for screenshotting (windowed builds).
    try:
        out_dir = diag.default_out_dir()
        reports = sorted(
            out_dir.glob('ZubCut-Support-Diag-*.txt'),
            key=lambda p: p.stat().st_mtime,
        )
        if reports and _sys.platform.startswith('win'):
            _os.startfile(str(reports[-1]))  # type: ignore[attr-defined]
    except Exception:
        pass
...
if __name__ == '__main__' and '--support-diag' in argv:
    _run_support_diag_and_exit()
